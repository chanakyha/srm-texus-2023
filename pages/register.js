...
const Register = () => {
  ...
  const FormVaildToast = (type) => {
    api[type]({
      message: 'Error',
      description:
        'Please fill all the fields and check the contact number',
    });
  };
  ...
  const addNewUserData = async () => {
    if (
      ...
      !values.year ||
      !values.department ||
      !values.contact || values.contact.length !== 10
    ) {
    ...
    <section className="w-full h-full bg-black py-[110px] font-montserrat text-white p-4 md:p-16">
        ...
        <div className='w-full max-w-2xl h-full mx-auto text-white  p-[30px] rounded-xl'>
                ...
                <div className="grid lg:grid-cols-2 gap-3" >
                        ...
                        ref={Year}
                        >
                            <option hidden className="text-[#858585] bg-white">Year of Study</option>
                ...
                <button className="bg-gradient-to-r from-[#FFEA2C] via-[#FF7A5D] to-[#FF0000] w-full max-w-2xl outline-none border-transparent my-2 h-14 pl-[15px] text-black font-bold rounded-md " onClick={addNewUserData}>Register</button>
        </div>
    </section>
    </>
  );
